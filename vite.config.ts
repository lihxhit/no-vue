  ...
  rollupOptions: {
    ...
    output: {
      ...
      */
      ...
      globals: {
        vue: 'Vue',
        // 'vue-router': 'VueRouter', // 引入 vue-router 全局变量，否则router.push将无法使用
...
const buildESAndLibOptions:BuildEnvironmentOptions= {
  ...
  rollupOptions: { // 自定义底层的 Rollup 打包配置
    ...
    // https://rollupjs.org/configuration-options/
    // 确保外部化处理那些你不想打包进库的依赖（作为外部依赖）
    external: ['vue', 'swiper/modules', 'swiper/vue', '@vuepic/vue-datepicker', '@vueuse/integrations/useQRCode', '@vueuse/core', 'seemly', 'qrcode'],
